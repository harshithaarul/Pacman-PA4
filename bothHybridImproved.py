...
class HybridAgent(CaptureAgent):
    ...
    def isOppClose(self, currentGameState):
        ...
        if(len(OppPacmanPos) != 0):
            if (self.onMySide(self.teammatePos, currentGameState) and self.onMySide(myPos, currentGameState)):
                for i in OppPositions:
                    if(distance.manhattan(myPos, i) <= (distance.manhattan(i, self.teammatePos) + 30)):
                        return True
            else:
                return True
        for i in OppPositions:
